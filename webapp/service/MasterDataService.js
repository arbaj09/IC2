  ...
  return {
    ...
    submitIntercoDocument: function (
      ...
      var oHdrPayload = {
        ...
        rec_preparer: (oHeader.recipientPreparer || "").trim(),
        approval_status  : "01"
      };
    ...
    saveDraftAndSimulate: function (
      ...
      var oHdrPayload = {
        ...
        // The header entity has a single shared taxcode column — it is
        // not split by side (unlike tax_gl/rec_tax_gl). It always holds
        // the Initiator's tax code, matching how _applyDraftToFormModel
        // reads it back (taxcode → initiatorTaxCode, never recipientTaxCode).
        // Switching this per sValidationSide caused the Recipient-side
        // save (run right after the Initiator's in onSaveDraft) to
        // overwrite it with the usually-empty recipientTaxCode, wiping
        // out the Initiator's tax code on every draft save.
        taxcode: (oHeader.initiatorTaxCode || "").slice(0, 2),
        rec_preparer: (oHeader.recipientPreparer || "").trim(),
      };
      ...
      // ---------------------------------------------------------
      // RECIPIENT VALIDATION — numbered like the Initiator side
      // (10, 20, 30, ...), continuing right after however many
      // Initiator items actually exist in the draft, instead of a
      // fixed reserved block. referencedocumentitem is the only
      // key on the item entity (not qualified by
      // initiator_recipient_ind), so the real Initiator count —
      // known once STEP 3A below queries the draft's items — is
      // what avoids a collision, not a guessed/reserved offset.
      // The system-generated Tax GL line gets "RT" instead of "R".
      // ---------------------------------------------------------
      function buildRecipientItemPayloads(iBaseCount) {
        (aRecipientLines || []).forEach(function (oLine, i) {
          var sInd = oLine.isTaxGLLine ? "RT" : "R";
          aItemPayloads.push(
            buildItemPayload(oLine, iBaseCount + i + 1, sInd),
          );
        });
      }

      if (sValidationSide === "R" && !sExistingDocId) {
        // Brand-new draft — nothing else exists yet, so start at "10".
        buildRecipientItemPayloads(0);
      }
      ...
      return (
        this._fetchCsrfToken(sRoot)
          ...
          // -----------------------------------------------------
          // STEP 2B  (existing draft only)
          // PATCH the header with the current form values. Branch A
          // above already sends oHdrPayload when creating the draft,
          // but re-saving an existing draft previously only touched
          // items (STEP 3A/3), so any header field edited after
          // reopening a draft (header text, dates, currency, tax
          // code/amount, etc.) was silently never persisted.
          // -----------------------------------------------------
          .then(function (oCtx) {
            if (!oCtx.isExistingDraft) {
              return oCtx;
            ...
            return new Promise(function (resolve) {
              jQuery.ajax({
                ...
                success: function (oData) {
                  ...
                  // Now that the real count of preserved (other-side)
                  // items is known, number the Recipient items to
                  // continue right after them — see buildRecipientItemPayloads
                  // above for why this replaces the old fixed offset.
                  if (sValidationSide === "R" && aItemPayloads.length === 0) {
                    buildRecipientItemPayloads(
                ...
                error: function (oXHR) {
                  ...
                  // Couldn't verify the real Initiator count — fall back
                  // to starting at "10" rather than leaving aItemPayloads
                  // empty (best effort; matches the fresh-draft case).
                  if (sValidationSide === "R" && aItemPayloads.length === 0) {
                    buildRecipientItemPayloads(0);
                  }
                  resolve(oCtx);
