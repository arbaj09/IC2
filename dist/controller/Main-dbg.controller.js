  ...
  function (
    ...
    return BaseController.extend("ZFI_INTERCO.controller.Main", {
      ...
      onInit: function () {
        ...
        this._loadJournalEntries({
          approval_status_codes: this.getView().getModel().getProperty(
    ...
    return mStatus[sCode] || sCode || "-";
      ...
      _loadJournalEntries: function (oFilters) {
        ...
        if (oFilters) {
          ...
          if (oFilters.createdby) {
            aFilterParts.push(
              "createdby eq '" +
                encodeURIComponent(oFilters.createdby).replace(/%20/g, " ") +
                "'",
            );
          }
      ...
      _fetchCreatedByVHData: function () {
        ...
        var sUrl =
          "/sap/opu/odata4/sap/zsb_interco_app/srvd/sap/zsd_interco_app/0001/" +
          "ZC_INTERCO_JE_HEADER?$select=createdby&$orderby=createdby";

        return this._fetchAllPages(sUrl)
          .then(function (aResults) {
            // Deduplicate by createdby
            var mSeen = {};
            var aUnique = aResults.filter(function (r) {
              if (!r.createdby || mSeen[r.createdby]) { return false; }
              mSeen[r.createdby] = true;
              return true;
            });
            oModel.setProperty("/referenceData/createdbyVHData", aUnique);
          })
          .catch(function (oError) {
      ...
      onSearchCreatedBySelect: function (oEvent) {
        var oContext = oEvent.getSource().getBindingContext();
        var sValue = oContext.getProperty("createdby");

        this.getView().getModel().setProperty("/search/createdby", sValue);
      ...
      onGoSearch: function () {
        ...
        var sCreatedBy = (oModel.getProperty("/search/createdby") || "").trim();
      ...
      _reopenDraftForEdit: function (oRow) {
        ...
        MasterDataService.getDraftForEdit(sDocId)
          .then(function (oDraft) {
            ...
            that._applyDraftToFormModel(oDraft.header, oDraft.items);
